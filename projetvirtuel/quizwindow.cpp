...
void quizwindow::on_pushButton_submit_clicked() {
    ...
    if (q.type == QuestionType::FillBlank) {
        ...
        if (chosen == -1) {
            QMessageBox::warning(this, "No Answer",
...
void quizwindow::showFeedback(bool correct,
                              const QString& explanation) {
    QString prefix = correct ? "✓  Correct!  " : "✗  Wrong!  ";
    ui->label_feedback->setText(prefix + explanation);
    ui->feedback_box->setStyleSheet(correct
                                        ? "QFrame { background:#052010; border:2px solid #22c55e; border-radius:10px; }"
                                        : "QFrame { background:#1a0505; border:2px solid #ef4444; border-radius:10px; }");
    ui->label_feedback->setStyleSheet(correct
                                          ? "QLabel { color:#22c55e; font-size:13px; background:transparent; }"
...
void quizwindow::finishQuiz() {
    ...
    qDebug() << "Quiz finished: score=" << score
             << "xp=" << totalXp
             << "user=" << m_username;

    // Save result to DB
    databasemanager::instance().saveQuizResult(
        m_username, result);
...
void quizwindow::on_pushButton_back_clicked() {
    if (!m_answers.isEmpty()) {
        auto btn = QMessageBox::question(this, "Exit Quiz",
