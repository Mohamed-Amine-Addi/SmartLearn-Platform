...
lessonviewerwindow::lessonviewerwindow(int courseId, const QString& courseTitle, const QString& username, QWidget* parent)
    : QWidget(parent)
    ...
    , m_username(username)
{
    ...
    // Mark course as started in the database
    databasemanager::instance().markCourseStarted(username, courseId);

    m_lessons = databasemanager::instance().getLessonsForCourse(courseId);
...
void lessonviewerwindow::showLesson(int index) {
    ...
    int barWidth = (900*(index+1))/m_lessons.size();
    ui->prog_fill->setFixedWidth(barWidth);
...
void lessonviewerwindow::updateButtons() {
    ...
    if (isLast) {
        ui->pushButton_next->setText("Take the Quiz →");
...
void lessonviewerwindow::on_pushButton_next_clicked() {
    if (m_currentIndex == m_lessons.size()-1) {
        // Mark course completed when all lessons are done
        databasemanager::instance().markCourseCompleted(m_username, m_courseId);
