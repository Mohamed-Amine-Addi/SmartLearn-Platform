...
void badgeswindow::buildBadgeCards() {
    ...
    ui->label_count->setText(
        QString("%1 / %2 unlocked").arg(unlocked).arg(badges.size()));
    ...
    for (const Badge& b : badges) {
        ...
        iconCircle->setStyleSheet(b.unlocked? "QFrame { background: #0d1f5c; border: 2px solid #1d4ed8; border-radius: 26px; }" : "QFrame { background: #0a0f1e; border: 1px solid #0f1629; border-radius: 26px; }");

        QLabel* iconLbl = new QLabel(b.icon, iconCircle);
        iconLbl->setGeometry(0, 0, 52, 52);
        iconLbl->setAlignment(Qt::AlignCenter);
        iconLbl->setStyleSheet(QString("color: %1; font-size: 20px; background: transparent;").arg(b.unlocked ? "#60a5fa" : "#1e3a5f"));
        ...
        nameLbl->setStyleSheet(QString("color: %1; font-size: 13px; font-weight: bold;"
                                       "background: transparent;").arg(b.unlocked ? "#e2e8f0" : "#334155"));
        QLabel* descLbl = new QLabel(b.description);
        descLbl->setStyleSheet(QString(
                                   "color: %1; font-size: 11px; background: transparent;").arg(b.unlocked ? "#334155" : "#1e3a5f"));
