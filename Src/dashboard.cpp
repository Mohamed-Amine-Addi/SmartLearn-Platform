...
void dashboard::loadUserStats() {
    QString firstName =
        databasemanager::instance().getUserFirstname(m_username);
    ...
    int started = databasemanager::instance().getCoursesStarted(m_username);

    //Count unlocked badges
    int unlockedBadges = 0;
    for (const Badge& b : databasemanager::instance().getUserBadges(m_username))
        if (b.unlocked) unlockedBadges++;

    //Welcome + sidebar
    ui->label_welcome->setText("Welcome back, " + firstName + "!");
    ui->label_username_side->setText(m_username);
    ui->label_level_side->setText("Level " + QString::number(level) + " learner");
    ...
    if (streak > 0) {
        ui->label_streak_title->setText(
            QString("%1-day streak — keep it going!")
                .arg(streak));
        ui->label_streak_sub->setText(
            QString("You last studied today · %1 XP total")
                .arg(xp));
    } else {
        ui->label_streak_title->setText("Start your streak today!");
        ui->label_streak_sub->setText("Complete a lesson each day to build your streak");
    }
    ...
    //Recent activity — show last 3 quiz results
    loadRecentActivity();
}
...
void dashboard::loadRecentActivity() {
    ...
    //Remove any old activity cards we added previously
    //They are direct children of main_area with objectName "activity_card"
    QList<QFrame*> oldCards = ui->main_area->findChildren<QFrame*>("activity_card");
    for (QFrame* c : oldCards) { c->deleteLater(); }
    ...
    for (int i = 0; i < count; i++) {
        ...
        card->setGeometry(24, 418 + i * 60, 752, 52);
        card->setStyleSheet(
            "QFrame{background:#070b14;border:1px solid"
            " #0f1629;border-radius:8px;}");

        //Score color
        QString col = e.score >= 80 ? "#22c55e" : e.score >= 50 ? "#f59e0b" : "#ef4444";

        //Score label
        QLabel* scoreLbl = new QLabel(
            QString::number(e.score) + "%", card);
        scoreLbl->setGeometry(12, 0, 50, 52);
        ...
        titleLbl->setGeometry(68, 8, 420, 18);
        ...
        QLabel* detLbl = new QLabel(
            QString("%1 correct of %2  ·  %3")
                .arg(e.correct).arg(e.total).arg(e.takenAt),card);
        detLbl->setGeometry(68, 28, 420, 16);
        ...
        xpLbl->setGeometry(580, 0, 100, 52);
