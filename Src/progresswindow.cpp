...
void progresswindow::buildHistoryCards() {
    ...
    for (const auto& entry : history) {
        ...
        QLabel* dateLbl = new QLabel(QString("%1 correct of %2  ·  %3").arg(entry.correct).arg(entry.total).arg(entry.takenAt));
        dateLbl->setStyleSheet("color: #334155; font-size: 11px; background: transparent;");
        ...
        // XP earned
        QLabel* xpLbl = new QLabel( "+" + QString::number(entry.xpEarned) + " XP");
        xpLbl->setStyleSheet(
